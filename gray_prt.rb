...
# => [motor_w_count,lm,rm,KP,KI,KD,i,threshold,gyro_offset]	-：黒寄り
    [    0, 1, 1,1,0.162, 0.04, 20,  1, 1], # pid0
    [ 1000, 1, 1,0.126,0.162, 0.04, 20,  1, 1], # pid1(1st_straight)
...
#var gomi
    ...
    $gray_value = color_calibration
		$gray_value = (($gray_value + $white_value)/2).to_i
        ...
        if iremono[pidchange][0] < motor_w_count #+ 15100
            ...
            if pidchange >= iremono.size - 1
              ...
              print "pidchange:",pidchange,">>sakimae\n"
				...
				#灰色探知
				print $color.reflect
				$color.reflect.times do |i|
					print "*"
				end
				puts

				if $color.reflect >= $gray_value && $color.reflect < $white_value
					Speaker.tone(:f4, 200)
				end
